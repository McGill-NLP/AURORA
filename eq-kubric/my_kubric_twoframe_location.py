...
#     locations = {
#         'closer': [ref_pos[0], max(spawn_y_min, ref_pos[1] - max_offset_y), obj_z],
#         'further away': [ref_pos[0], min(spawn_y_max, ref_pos[1] + max_offset_y), obj_z],
#         'further left': [max(spawn_x_min, ref_pos[0] - max_offset_x), ref_pos[1], obj_z],
#         'further right': [min(spawn_x_max, ref_pos[0] + max_offset_x), ref_pos[1], obj_z],
#     }
...
def gen_caption(obj_name, obj_scale, ref_obj_name, ref_obj_scale, location):
    ...
    # import pdb; pdb.set_trace()
    # obj_size_exp = get_size(obj_scale)
    # ref_obj_size_exp = get_size(ref_obj_scale)
    # verb_exp = random.choice(['is', 'is located', 'is placed'])
    if location == 'front':
        location_exp = 'in front of'
    elif location == 'behind':
        location_exp = 'behind'
    elif location == 'left':
        location_exp = random.choice(['on the left of', 'on the left side of', 'on the left hand of'])
    elif location == 'right':
        location_exp = random.choice(['on the right of', 'on the right side of', 'on the right hand of'])
    elif location == 'on':
        location_exp = random.choice(['on the top of', 'above'])
    
    # elif location == 'further right':
    #     location_exp = 'further right'
    # elif location == 'further left':
    #     location_exp = 'further left'
    # elif location == 'further away':
    #     location_exp = 'further away'
    # elif location == 'closer':
    #     location_exp = 'closer'

    edit_verb = random.choice(['put', 'shift', 'change', 'place'])
    # caption = f'the{obj_size_exp} {obj_name} {verb_exp} {location_exp} the{ref_obj_size_exp} {ref_obj_name}'
    if edit_verb in ['put', 'place']:
        caption = f'{edit_verb} the {obj_name} {location_exp} the {ref_obj_name}'
    else:
        caption = f'{edit_verb} the position of the {obj_name} {location_exp} the {ref_obj_name}'
...
ref_location1, ref_location2 = random.sample(LOC_SET, 2)
...
ref_obj_pos = ref_object.position
ref_obj_z_up = get_obj_z_up(ref_object.bounds, ref_object.scale)
new_obj_pos = get_new_pos(obj.bounds, obj.scale, ref_location2, ref_obj_pos, ref_obj_z_up, ref_object, rng)
...
trails = 0
while not check_ok(obj, obj.position, SPAWN_REGION_OBJ):
    trails += 1
    obj.position = get_new_pos(obj.bounds, obj.scale, ref_location2, ref_obj_pos, ref_obj_z_up, ref_object, rng)
...
caption_2 = gen_caption(new_object_name, obj.metadata["scale"], ref_object_name, ref_object.metadata["scale"], ref_location2)
print(caption_2)
